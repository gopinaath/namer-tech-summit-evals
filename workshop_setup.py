"""Bedrock setup for the NAMER Tech Summit workshop notebooks.

All four notebooks start with the same three jobs: make sure the kernel is one we're allowed
to install into, resolve Bedrock credentials from `.env`, and check which Claude models this
account can actually reach. That is ~330 lines of defensive plumbing whose value is entirely
in the messages it prints when something is wrong — and none of it is what the workshop is
about. It lives here so the notebooks can start with the material instead of the machinery.

    from workshop_setup import connect

    _setup = connect()
    client = _setup.client

`connect()` is safe to call repeatedly: it re-reads `.env`, re-checks the install and
re-pings every model each time. That matters, because the intended flow when credentials are
missing is *the cell writes a `.env`, you fill it in, you re-run the cell*.

Everything it returns, all of which the notebooks bind as globals:

    client              a ready anthropic.AnthropicBedrock, used by every later cell
    make_client()       build another one (different timeout, or a fresh connection pool)
    MODEL               Sonnet 5    — the workhorse
    FAST_MODEL          Haiku 4.5   — cheap and quick; the agent and judge default
    BIG_MODEL           Opus 5      — the largest, for the Part 7 comparison
    FABLE_5_1_MODEL     Fable 5.1   — another point on the curve
    OPTIONAL_MODELS     the three above FAST_MODEL, i.e. the ones allowed to be missing
    AVAILABLE_MODELS    of those, the ones this account really can call — what sweeps use
    short_model_name()  long inference-profile ID -> a name that fits in a table
    auth_mode           "api-key" or "iam"
    region token profile   the resolved credentials, reused by the Converse notebook

If setup fails, read the message rather than this file: every failure path here prints what
to change and where. `SETUP.md` lists them all with fixes.
"""
...
# Everything except FAST_MODEL is optional: the notebooks run start to finish on Haiku alone.
# connect() pings each of these and keeps the ones your account can actually reach, so a
# missing grant costs you a table row rather than an hour of debugging.
OPTIONAL_MODELS = [MODEL, BIG_MODEL, FABLE_5_1_MODEL]

# Resolved by connect(). Module-level rather than local because make_client() is handed to
# the notebooks and may be called from any later cell, long after connect() returned.
AUTH_MODE = None
_region = ""
_token = ""
_profile = ""
...
# ── Is this kernel safe to pip install into? ──────────────────────────────────
...
def _require_installable_kernel():
    """Refuse to touch a shared Python. Checked on every connect() rather than once at
    import, so that fixing the kernel and re-running the cell re-checks it."""
...
def _require_anthropic():
    """Install the SDK if it's missing, then hand the module back.

    Imported here rather than at module top level because on a hosted notebook the package
    may not exist until the line above has run."""
    ...
    if importlib.util.find_spec("anthropic") is None:
        ...
        # A fresh install lands in a directory the import system has already scanned.
...
def _resolve_env_file():
    """Prefer a .env in the working folder, then the nearest one walking up (so a shared
    .env one level up still works, which is what the demo copy in demo/ relies on). If none
    exists, create one right here."""
...
def _load_credentials():
    """Read .env into the environment and work out which credential type we have."""
    global AUTH_MODE, _region, _token, _profile

    env_file = _resolve_env_file()
    if not env_file.exists():
        env_file.write_text(_ENV_TEMPLATE)
        print(f"Created {env_file.name} in {env_file.parent} — open it, add your Bedrock "
    ...
    if _token:
        AUTH_MODE = "api-key"
    elif (_profile or os.environ.get("AWS_ACCESS_KEY_ID")
          or (pathlib.Path.home() / ".aws" / "credentials").exists()):
        AUTH_MODE = "iam"
    else:
        _needs_credentials(
...
def make_client(timeout=900.0, max_retries=4):
    ...
    if AUTH_MODE is None:
        # Reachable if connect() failed part-way and something called this afterwards. Say so,
        # rather than handing back a client with an empty region that fails obscurely later.
        raise RuntimeError("No credentials resolved yet — run the setup cell at the top of "
                           "the notebook (or workshop_setup.connect()) first.")
...
def _check_models(anthropic):
    """Ping the models and return the ones this account can really call.
    ...
    probe = make_client(timeout=30.0, max_retries=1)

    def ping(model_id):
        """True if this exact model ID answers for these credentials in this region.

        `bedrock-runtime` reports an unusable model as a 400, not a 404 — "The provided
        model identifier is invalid" for an ID that doesn't exist in this region, and
        "Invocation ... with on-demand throughput isn't supported" for a plain model ID that
        needs an inference profile. Both mean "not usable here", so both count as False
        rather than an error."""
        try:
            probe.messages.create(model=model_id, max_tokens=1,
    ...
    try:
        agent_model_ok = ping(FAST_MODEL)
    except anthropic.PermissionDeniedError:
        # AccessDenied is ambiguous on Bedrock: the credential may lack bedrock:InvokeModel,
        # or it may be fine and this particular model just isn't granted. Name both —
        # guessing one sends people to the wrong console page.
    ...
    # Model access is granted per account, one model at a time — so the model powering the
    # agent working tells you nothing about the others. Check them all now, for three tokens,
    # rather than discovering it in a model sweep an hour from now.
    def reachable(model_id):
        try:
            return ping(model_id)
        except Exception:
            # The credential is already proven by this point, so anything else — an
            # AccessDenied for this specific model, a throttle, a dropped connection — means
            # "can't use it today". Treat it as unavailable rather than failing setup over an
            # optional model.
            return False

    return [FAST_MODEL] + [m for m in OPTIONAL_MODELS if reachable(m)]


# ── The one call the notebooks make ───────────────────────────────────────────

def connect():
    """Check the kernel, resolve credentials, ping the models, return a ready client.

    Safe to re-run. Raises SystemExit with an actionable message on any failure — which
    IPython renders as a single line, not a traceback, so the message is what people see."""
    _require_installable_kernel()
    anthropic = _require_anthropic()
    _load_credentials()

    available = _check_models(anthropic)
    missing = [m for m in OPTIONAL_MODELS if m not in available]

    _status(True, f"Connected to Claude on Bedrock — {_region}, auth: {AUTH_MODE}. "
                  f"Models available: "
                  f"{', '.join(short_model_name(m) for m in available)}.")
    if missing:
        # Deliberately not named after one notebook's Part 7: this runs for every notebook,
        # and only one of them has a Part 7. Say what is true everywhere instead.
        print(f"Note: {', '.join(short_model_name(m) for m in missing)} not available to this "
    ...
    return SimpleNamespace(
        client=make_client(),          # the working client used by everything below
        make_client=make_client,
        MODEL=MODEL,
        FAST_MODEL=FAST_MODEL,
        BIG_MODEL=BIG_MODEL,
        FABLE_5_1_MODEL=FABLE_5_1_MODEL,
...
if __name__ == "__main__":
    # `python workshop_setup.py` is a credentials smoke test that needs no notebook.
    connect()
